...
class Text_Parser:
    ...
    def __init__(self,Dataset_path):
        if os.path.exists(Dataset_path):
            ...
            assert os.path.lexists(Dataset_path),'Error: dataset path does not exists!'

    """
    This method returns a dictionary containing the text_data of the dataset
    ...
    def utterance_parser(self):
        ...
        Gender_list = []
        Transcription_list= []
        listofSessions = ["Session1","Session2","Session3","Session4","Session5"]

        for Session in listofSessions:

            path = self.datasetPath+"/"+Session+"/dialog/transcriptions/"
            ...
            for utt_file in contained_files:
                with open(utt_file,"r") as inputfile:
                    for line in inputfile.readlines():
                        ...
                        #utterance_key must start with Ses and not contains MX.. or FX..
                        if( utterance_key.startswith("Ses") and ( "MX" not in utterance_key) and ( "FX" not in utterance_key) ):
                            #delete newline on transcription end and also delete space at transcription's start
                            transcription = transcription[1:len(transcription)-1]
                            gender = utterance_key[len(utterance_key)-4:len(utterance_key)-3]
                            #add all to dict
                            text_attr_dict['Utterance-Id'].append(utterance_key)
